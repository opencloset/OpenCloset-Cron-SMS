    ...
    $w = OpenCloset::Cron::Worker->new(
        ...
        cb        => sub {
            ...
            while ( my $order = $order_rs->next ) {
                ...
                my $msg = sprintf(
                    qq{[열린옷장] 내일은 %d일에 대여하신 의류 반납일입니다.

< 반납안내 >
1. 택배 반납시 : 대여기간 내에 적당한 상자에 담아서 보내주세요. (택배비 본인 부담)
 - 주소: 서울시 광진구 아차산로 213 (화양동, 웅진빌딩) 403호 (우.05019)
 - 전화: 02-6929-1020

2. 방문 반납시 :  4층 엘리베이터 앞 노란 무인반납함
...
my $cron = OpenCloset::Cron->new(
    ...
    workers => [
        $worker1, $worker2, $worker3, $worker4, $worker5, $worker7, $worker8
    ],
);
$cron->start;
